...
#SBATCH -o output_pers_SPADES1_%j.txt
#SBATCH -e errors_pers_SPADES1_%j.txt
...
INPUTDIR=/scratch/project_2005863/metatranscriptomics_2025/Theo_analysis/I_pers_mapped/unmapped

OUTPUTDIR=/scratch/project_2005863/metatranscriptomics_2025/Theo_analysis/I_pers_metagenome_assembly

# Create output directories (if they don’t exist)
mkdir -p $OUTPUTDIR

for f in $INPUTDIR/*_I_pers_unmapped_R1.fastq.gz
do
    echo "Processing $f"
    ...
    base="${filename%*_I_pers_unmapped_R1.fastq.gz}"
...
metaspades.py -1 $INPUTDIR/"${prfx}"_I_pers_unmapped_R1.fastq.gz -2 $INPUTDIR/"${prfx}"_I_pers_unmapped_R2.fastq.gz -o $OUTPUTDIR/"${prfx}" -t $SLURM_CPUS_PER_TASK -m $SLURM_MEM_PER_NODE

...
#SBATCH --job-name=viral_reads_extraction
#SBATCH --error=virus_read.err
#SBATCH --output=slurm_logs/extract_multi_viruses_%j.out
...
# Input directories
KRAKEN_DIR="/scratch/project_2005863/metatranscriptomics_2025/Theo_analysis_metatranscriptomics/persulcatus_kraken_results"
FASTQ_DIR="/scratch/project_2005863/metatranscriptomics_2025/Theo_analysis_metatranscriptomics/I_pers_mapped/unmapped"
OUT_DIR="/scratch/project_2005863/metatranscriptomics_2025/Theo_analysis_metatranscriptomics/extracted_viruses_persul"
...
while IFS=$'\t' read -r virus taxid; do
    ...
    for k in ${KRAKEN_DIR}/*.kraken.out.gz; do
        ...
        fq1=$(find "$FASTQ_DIR" -type f -name "*${s}*_I_pers_unmapped_R1.fastq.gz" | head -n1)
        fq2=$(find "$FASTQ_DIR" -type f -name "*${s}*__I_pers_unmapped_R2.fastq.gz" | head -n1)
